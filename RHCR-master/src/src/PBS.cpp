...
bool has_footprint_overlap(const BasicGraph& G, int loc1, const Footprint& fp1, int theta1, int loc2, const Footprint& fp2, int theta2)
{
    if (loc1 < 0 || loc2 < 0) return false;
    auto cells1 = G.get_footprint_locations(loc1, fp1, theta1);
    auto cells2 = G.get_footprint_locations(loc2, fp2, theta2);
    for (int c1 : cells1)
    {
        for (int c2 : cells2)
        {
...
void PBS::find_conflicts(list<Conflict>& conflicts, int a1, int a2)
{
	...
	{ 
		...
		for (size_t timestep = 0; timestep < min_path_length; timestep++)
		{
            ...
            int theta1 = max(0, paths[a1]->at(timestep).orientation);
			int loc2 = paths[a2]->at(timestep).location;
            int theta2 = max(0, paths[a2]->at(timestep).orientation);
			if (has_footprint_overlap(G, loc1, agent_footprints[a1], theta1, loc2, agent_footprints[a2], theta2))
			{
		...
		if (paths[a1]->size() != paths[a2]->size())
		{
            ...
            int theta1 = max(0, paths[a1_]->back().orientation);
			for (size_t timestep = min_path_length; timestep < paths[a2_]->size(); timestep++)
			{
				int loc2 = paths[a2_]->at(timestep).location;
                int theta2 = max(0, paths[a2_]->at(timestep).orientation);
				if (has_footprint_overlap(G, loc1, agent_footprints[a1_], theta1, loc2, agent_footprints[a2_], theta2))
				{
	...
	else
	{
		...
		for (int timestep = 0; timestep < size1; timestep++)
		{
			...
			for (int i = max(0, timestep - k_robust); i <= min(timestep + k_robust, size2 - 1); i++)
			{
                ...
                int theta2 = max(0, paths[a2]->at(i).orientation);
				if (has_footprint_overlap(G, loc1, agent_footprints[a1], theta1, loc2, agent_footprints[a2], theta2))
				{
			...
			if (k_robust == 0 && timestep < size1 - 1 && timestep < size2 - 1) // detect edge (swap) conflicts
		{
			...
			int theta1_cur = max(0, paths[a1]->at(timestep).orientation);
			int theta2_cur = max(0, paths[a2]->at(timestep).orientation);
			int theta1_next = max(0, paths[a1]->at(timestep + 1).orientation);
			int theta2_next = max(0, paths[a2]->at(timestep + 1).orientation);
			bool fp_swap = (!center_swap &&
							has_footprint_overlap(G, loc1_next, agent_footprints[a1], theta1_next, loc2_cur,  agent_footprints[a2], theta2_cur) &&
							has_footprint_overlap(G, loc2_next, agent_footprints[a2], theta2_next, loc1_cur,  agent_footprints[a1], theta1_cur));
...
bool PBS::generate_root_node()
{
    ...
    for (int i = 0; i < num_of_agents; i++) 
	{
        ...
        t = std::clock();
        std::cout << "  Running path planner..." << std::endl;
        ...
        if (path.empty())
        {
            std::cout << "NO SOLUTION EXISTS for agent " << i << " from " << start_location 
                      << " (orient=" << starts[i].orientation << ") to " 
                      << goal_locations[i][0].first << std::endl;
            return false;
        }
    ...
    if (!lazyPriority)
    {
        std::cout << "  Finding consistent paths..." << std::endl;
        if(!find_consistent_paths(dummy_start, -1))
        {
            std::cout << "  find_consistent_paths failed!" << std::endl;
            return false;
        }
    }
