...
int main(int argc, char** argv) 
{
	...
	if (vm["scenario"].as<string>() == "KIVA")
	{
        ...
        if (fp_shape == "sideloader") {
            footprints.assign(system.num_of_drives, Footprint::SideLoader());
            cout << "Using SIDELOADER footprint (3x1 horizontal body, fork at center)" << endl;
        } else if (fp_shape == "forklift") {
            int fp_h = vm["footprint_height"].as<int>();
            footprints.assign(system.num_of_drives, Footprint::Forklift(fp_h));
            cout << "Using FORKLIFT footprint: length=" << fp_h << " (front=strip, body=backward)" << endl;
        } else if (fp_shape == "rect") {
