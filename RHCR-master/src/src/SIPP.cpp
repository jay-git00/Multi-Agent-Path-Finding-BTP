...
Path SIPP::updatePath(const BasicGraph& G, const SIPPNode* goal)
{
    ...
    while (true)
    {
        ...
        else
        {
            ...
            if (degree == 3) // turn right or turn left (takes 3 timesteps)
            {
                // Smooth interpolation for visualizer if needed, but here we just wait
                path[t] = State(prev->state.location, t, curr->state.orientation);
                t++;
            }
            else if (degree == 6) // turn back (takes 6 timesteps)
            {
                path[t] = State(prev->state.location, t, (prev->state.orientation + 1) % 4); // turn right intermediate
                t++;
    ...
    while (!focal_list.empty())
    {
        ...
        for (int orientation = 0; orientation < 4; orientation++) // move
        {
            ...
            if (degree > std::get<1>(curr->interval) - curr->state.timestep) // don't have enough time to turn
                continue;
            if (degree > 0 && !G.isRotationValid(curr->state.location)) // swept volume hits an obstacle!
                continue;
            ...
            int from_theta = curr->state.orientation < 0 ? 0 : curr->state.orientation;
            int to_theta = curr->state.orientation < 0 ? 0 : orientation;
            for (auto interval : rt.getSafeIntervals(curr->state.location, location, from_theta, to_theta, min_timestep, std::get<1>(curr->interval) + 1))
            {
                if (curr->state.orientation < 0)
        ...
        // Add Reverse Move: move backward without changing orientation
        if (curr->state.orientation >= 0)
        {
            int rev_dir = (curr->state.orientation + 2) % 4;
            if (G.weights[curr->state.location][rev_dir] < WEIGHT_MAX - 1)
            {
                int location = curr->state.location + G.move[rev_dir];
                if (G.isFootprintValidAtState(location, curr->state.orientation))
                {
                    double h_val = compute_h_value(G, location, curr->goal_id, goal_location);
                    if (h_val < INT_MAX)
                    {
                        int min_timestep = curr->state.timestep + 1;
                        for (auto interval : rt.getSafeIntervals(curr->state.location, location, curr->state.orientation, curr->state.orientation, min_timestep, std::get<1>(curr->interval) + 1))
                        {
                            generate_node(interval, curr, G, location, min_timestep, curr->state.orientation, h_val);
                        }
                    }
                }
            }
        ...
        if(rt.use_cat) // wait to the successive interval
        {
            ...
            Interval interval;
            bool found = rt.findSafeInterval(interval, location, orientation >= 0 ? orientation : 0, min_timestep);
            if (found)
            {
				if (curr->state.orientation < 0)
				{
