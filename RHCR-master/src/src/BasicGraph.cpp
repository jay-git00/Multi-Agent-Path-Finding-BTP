...
bool BasicGraph::isFootprintValidAtState(int location, int orientation) const
{
    ...
    for (const auto& offset : rotated_fp.offsets)
    {
        ...
        {
            // STRICT: The tail/body MUST stay on Travel or Home cells ONLY.
            // It CANNOT be on Endpoints (racks) or Obstacles.
            if (types[cell] != "Travel" && types[cell] != "Home")
                return false;
        }
    }
    return true;
...
bool BasicGraph::isRotationValid(int location) const
{
    ...
    for (int cell : swept)
    {
        // During a turn, NO part of the swept volume can hit an Obstacle or an Endpoint.
        // It must be entirely within Travel or Home space (plus the center can be wherever, but
        // since rotation implies turning IN PLACE, if it's currently at an Endpoint, the center is fine,
        // but wait! If the center is at an Endpoint, is it allowed to turn?
        // Let's say if it is the center cell, we skip the check because we know the center is valid.
        if (cell == location)
            continue;
