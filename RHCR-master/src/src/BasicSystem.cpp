...
void BasicSystem::update_start_locations()
{
    for (int k = 0; k < num_of_drives; k++)
    {
        ...
        if (k == 0) cout << "Agent 0 new start: " << starts[k].location << " at t=" << timestep << " paths[0].size=" << paths[0].size() << endl;
    }
}
    ...
    for (int t = start_timestep; t <= end_timestep; t++)
    {
        for (int k = 0; k < num_of_drives; k++)
        {
				...
				curr.timestep >= goal_locations[k].front().second) // the agent finish its current task
            {
                ...
                cout << "Agent " << k << " finished task at loc " << curr.location << " at local t=" << t << endl;
            }
            ...
            if (t > 0)
            {
                ...
                else if (consider_rotation)
                {
                    ...
                    {
                        // Check forward move: agent moves in its facing direction
                        bool is_forward = (prev.location + G.move[prev.orientation] == curr.location);
                        // Check reverse move: agent backs up (opposite direction) while keeping orientation
                        int rev_dir = (prev.orientation + 2) % 4;
                        bool is_reverse = (prev.location + G.move[rev_dir] == curr.location);
    ...
    solver.agent_footprints = agent_footprints;
		...
		bool sol = solver.run(starts, goal_locations, time_limit);
  		...
  		else
  		{
            ...
            whca.window = INT_MAX;
            whca.screen = screen;
            whca.agent_footprints = agent_footprints;
            sol = whca.run(starts, goal_locations, time_limit);
            assert(sol);
  			update_paths(whca.solution);
  		}
	}
	 else // PBS or ECBS
	 {
		 ...
		 if (hold_endpoints || useDummyPaths)
		 {
             ...
             cout << "Before update_paths, planned_paths[0][0] is " << (planned_paths[0].empty() ? -1 : planned_paths[0][0].location) << endl;
